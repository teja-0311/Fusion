...
def me(request):
    """
    GET /api/auth/me
    Returns current user info, designations and accessible modules.
    Called by validateauth.jsx on every page load to verify session.
    """
    user = request.user

    # Get designations
    designation_list = list(
        HoldsDesignation.objects.filter(working=user).values_list('designation', flat=True)
    )
    designation_info = []
    for did in designation_list:
        try:
            d = Designation.objects.get(id=did)
            designation_info.append(str(d.name))
        except Designation.DoesNotExist:
            pass

    # Build accessible modules based on designations
    # These match the module IDs in sidebarContent.jsx
    all_modules = [
        'complaint_management', 'fts', 'home', 'mess_management',
        'visitor_hostel', 'hostel_management', 'department', 'gymkhana',
        'iwd', 'phc', 'spacs', 'placement_cell', 'purchase_and_store',
        'rspc', 'inventory_management', 'program_and_curriculum',
        'course_registration', 'examinations', 'other_academics', 'hr',
        'course_management', 'patent_management',
    ]
    accessible_modules = {module: True for module in all_modules}

    # Get roll number if student
    roll_no = None
    try:
        if hasattr(user, 'extrainfo') and user.extrainfo.user_type == 'student':
            roll_no = str(user.extrainfo.student.id)
    except Exception:
        pass
...
def notifications_list(request):
    ...
    notifications = serializers.NotificationSerializer(
        request.user.notifications.all()[:20], many=True
    ).data
